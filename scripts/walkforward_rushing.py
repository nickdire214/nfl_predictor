"""Step 44: walk-forward evaluation for the RB rushing-yards Ridge model.

Read-only evidence-gathering, no saved model. For each test season 2022-2025,
trains StandardScaler+Ridge(alpha=1.0) on prior seasons and evaluates on the
test season, against naive-L8 and naive-season baselines, on ALL test rows and
on the prop-relevant slice (carries_l8 >= 8). Run with the venv's python:
    venv\\Scripts\\python.exe scripts\\walkforward_rushing.py
"""
...
from src.models.train_rushing import FEATURE_COLS, TARGET, _bias, _mae, preprocess_rushing_matrix

SEASONS = [2022, 2023, 2024, 2025]
PROP_RELEVANT_MIN_CARRIES_L8 = 8


df = pd.read_parquet(FEATURES_DIR / "rushing_matrix.parquet")
df = preprocess_rushing_matrix(df)
...
for season in SEASONS:
    ...
    naive_l8_pred = test["rushing_yards_l8"].values
    naive_season_pred = test["rushing_yards_std"].values
    y_test_vals = y_test.values

    prop_mask = (test["carries_l8"] >= PROP_RELEVANT_MIN_CARRIES_L8).values
