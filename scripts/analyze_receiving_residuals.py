"""Step 27: receiving calibration + residual diagnostics.

Read-only evidence-gathering, no pipeline changes. Run with the venv's python:
    venv\\Scripts\\python.exe scripts\\analyze_receiving_residuals.py
"""
...
for season in SEASONS:
    ...
    records.append(test[[
        ...
        "raw_residual", "cal_residual", "targets_l8", "position",
    ]])
...
section(f"Part B: Pooled residual distribution, prop-relevant slice (targets_l8 >= {PROP_MIN_TARGETS_L8}), "
...
section("Heteroskedasticity: residual std by targets_l8 quartile (prop-relevant slice)")
prop_data["targets_quartile"] = pd.qcut(prop_data["targets_l8"], 4, duplicates="drop")
print(prop_data.groupby("targets_quartile", observed=True)[residual_col].agg(["count", "std"]).to_string())

section("Heteroskedasticity: residual std by position (prop-relevant slice)")
print(prop_data.groupby("position", observed=True)[residual_col].agg(["count", "std"]).to_string())


section("Pooled-residual p10/p90 of (prediction + residual), by predicted-value quartile")
print(f"{'quartile':<20} {'median_pred':>12} {'p10':>10} {'p90':>10}")
for quartile, group in prop_data.groupby("pred_quartile", observed=True):
    median_pred = group[pred_col].median()
    p10 = median_pred + np.quantile(residuals, 0.10)
    p90 = median_pred + np.quantile(residuals, 0.90)
    print(f"{str(quartile):<20} {median_pred:>12.2f} {p10:>10.2f} {p90:>10.2f}")
