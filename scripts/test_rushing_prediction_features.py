"""Hard-assertion tests for build_rushing_prediction_features (src/features/rushing.py).
...
nonzero if any assertion fails. Run with the venv's python:
    venv\\Scripts\\python.exe scripts\\test_rushing_prediction_features.py
"""
...
from src.models.train_rushing import ROLLING_COLS
...
section("(a) Replay equivalence: build_rushing_prediction_features vs rushing_matrix.parquet")

rushing_matrix = pd.read_parquet(FEATURES_DIR / "rushing_matrix.parquet")

for week in (5, 14):
    pred_df = build_rushing_prediction_features(2025, week)
    actual_df = rushing_matrix[(rushing_matrix["season"] == 2025) & (rushing_matrix["week"] == week)]
    ...
    # Predicted roster is "each team's RB room from its most recent prior game"
    # — week-to-week churn (returns from rest/injury, healthy scratches) means
    # pred and actual row sets are not identical. Feature equivalence is checked
    # over the intersection; coverage is informational.
    ...
    print(f"PASS: week {week} ({len(common_keys)} common rows) — all feature columns match rushing_matrix.parquet")
...
normal_result = build_rushing_prediction_features(2025, 10)
...
try:
    ...
    leakage_result = build_rushing_prediction_features(2025, 10)
finally:
...
# `as_of` is a prediction-time metadata column (not in rushing_matrix); the
# column set must otherwise match the matrix exactly.
METADATA_COLS = {"as_of"}

future_df = build_rushing_prediction_features(2025, 23)

extra = set(future_df.columns) - set(rushing_matrix.columns)
missing = set(rushing_matrix.columns) - set(future_df.columns)
...
print(f"PASS: build_rushing_prediction_features(2025, 23) ran without error, "
